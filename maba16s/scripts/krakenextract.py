...
def extract_reads(name, krakenfile, genusID, fastqin, outdir):
    os.system(f'mkdir -p {outdir}')
    outfile = outdir + "/" + str(genusID) + "_" + name + ".fasta" 
    os.system(f'extract_kraken_reads.py -k "{krakenfile}" -o "{outfile}" -s "{fastqin}" -t {genusID}')
...
def read_krakenreport(file, krakenfile, fastqin, outdir):
    ...
    for i in df.iterrows():
        info = i[1]
        name = re.sub(r'[^A-Za-z0-9]', '', info[5])
        genusID = info[4]
        extract_reads(name, krakenfile, genusID, fastqin, outdir)
